...
<br>
  <p class=standard id=try><font face="Verdana" size="2">2. Dengan ini adalah diberitahu 
    ...
    <%if jab="112" then %>
    Jabatan Pelesenan,Tingkat 1, 
    <%else%>
    Jabatan Kesihatan, Tingkat 7, 
    <%end if%>
    Majlis Perbandaran Seberang Perai,Jalan Perda Utama, Bandar Perda,14000 Bukit 
...
<%	if jab = "112" then
	...
	s = " select upper(nama) nama from payroll.paymas where no_pekerja = '70031' "		
	...
	s = " select upper(nama) nama from payroll.paymas where no_pekerja = '08752' "	
	Set nms = objConn.Execute(s)
...
Penolong Pengarah Pelesenan,<%	else%>
Timbalan Pengarah Perkhidmatan Kesihatan,<%end if%>
