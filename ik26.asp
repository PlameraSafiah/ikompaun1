<%response.cookies("ikmenu") = "ik26.asp"%>
<!-- '#INCLUDE FILE="ik.asp" -->
...
<form name=myform method="POST" action="ik26.asp">
...
%>
 <table width="50%" cellpadding="1" cellspacing="5" class="hd">
  <tr> 
    ...
    <% '------mimi buka max length 12 : Pn. raja 24july 2018  %>
...
<%
	...
	if proses = "Cari" or rujuk <> "" or nama <> "" or add <> "" then
		...
		d = d & " order by to_date(tkh_masuk,'dd/mm/yyyy') asc "  			
 ...
 <table width="50%" cellpadding="1" cellspacing="5" class="hd1">
  <tr align="right">
    ...
    <td colspan="6">
      ...
      <a href="ik26.asp?page=1&bilangan=0&ms=pre&drujuk=<%=rujuk%>&dnama=<%=nama%>&dadd=<%=add%>&proses=Cari">
		<img name="firstrec" border="0" src="images/firstrec.jpg" width="20" height="20" alt="Halaman Mula"></a>  
      <% End If %>
      <% If iPageCurrent <> 1 Then%>
      <a href="ik26.asp?page=<%= iPageCurrent - 1 %>&bilangan=<%=bil-count-iPageSize%>&ms=pre&drujuk=<%=rujuk%>&dnama=<%=nama%>&dadd=<%=add%>&proses=Cari">
      ...
      <a href="ik26.asp?page=<%= iPageCurrent + 1 %>&bilangan=<%=bil%>&ms=next&drujuk=<%=rujuk%>&dnama=<%=nama%>&dadd=<%=add%>&proses=Cari">
      	<img name="next" border="0" src="images/next.jpg" width="20" height="20" alt="Rekod Seterusnya"></a> 
      <% End If 
	  If iPageCurrent < iPageCount Then
	  bil = (iPageCount - 1) * iPageSize %>
      <a href="ik26.asp?page=<%=iPageCount %>&bilangan=<%=bil%>&ms=next&drujuk=<%=rujuk%>&dnama=<%=nama%>&dadd=<%=add%>&proses=Cari">
...
<td width="200" class="hd1">Alamat </td>
...
<td align="left"><%=objRs2("nama")%></td>
<td align="left"><%=objRs2("alamat1")%>&nbsp;<%=objRs2("alamat2")%>&nbsp;<%=objRs2("alamat3")%>&nbsp;<%=objRs2("daerah")%></td>
...
'    ff = ff & " from hasil.bil2 where no_akaun = '"& no &"' and status is null "
	...
	if not objRsf.eof then
		...
		totalamq = totalamq + amq

  %>
    
    <td colspan="3"><font color="#FF0066" size="-2">Bayaran Telah Dibuat Pada <%=tkhq%>
	  di Stesyen <%=stesyen%>. Jumlah: RM <%=amq%>. <br>No Resit:<%=resitq%> . Data Belum DiSahkan </font></td>
  <%else%>
...
%> 
 ...
 <td align="right"><b>RM <%=FormatNumber(total_besaq,2)%></b></td></tr>
  <tr>
    <tr>
 <td colspan="6" align="right"><b>Jumlah Keseluruhan</b></td>
 <td align="right"><b>RM
<% if amaun_bayart <> "" or amaun_bayart <>0 then %>
<%=FormatNumber(of2("amaun_bayart"),2)%>
<% else %>
0.00
<%end if %>
</b></td></tr>
