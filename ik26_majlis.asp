<%response.cookies("ikmenu") = "ik26_majlis.asp"%>
<!-- '#INCLUDE FILE="ik.asp" -->
...
<form name=myform method="POST" action="ik26_majlis.asp">
...
<td><input type="text" name="no_rujuk2" value="<%=rujuk%>" onFocus="nextfield='nama';" maxlength="12" size="12"></td>
</tr>
<script>
 ...
 <table width="50%" cellpadding="1" cellspacing="5" class="hd1">
  <tr align="right">
    ...
    <td colspan="6">
      ...
      <a href="ik26_majlis.asp?page=1&bilangan=0&ms=pre&drujuk=<%=rujuk%>&dnama=<%=nama%>&dadd=<%=add%>&proses=Cari">
		<img name="firstrec" border="0" src="images/firstrec.jpg" width="20" height="20" alt="Halaman Mula"></a>  
      <% End If %>
      <% If iPageCurrent <> 1 Then%>
      <a href="ik26_majlis.asp?page=<%= iPageCurrent - 1 %>&bilangan=<%=bil-count-iPageSize%>&ms=pre&drujuk=<%=rujuk%>&dnama=<%=nama%>&dadd=<%=add%>&proses=Cari">
      ...
      <a href="ik26_majlis.asp?page=<%= iPageCurrent + 1 %>&bilangan=<%=bil%>&ms=next&drujuk=<%=rujuk%>&dnama=<%=nama%>&dadd=<%=add%>&proses=Cari">
      	<img name="next" border="0" src="images/next.jpg" width="20" height="20" alt="Rekod Seterusnya"></a> 
      <% End If 
	  If iPageCurrent < iPageCount Then
	  bil = (iPageCount - 1) * iPageSize %>
      <a href="ik26_majlis.asp?page=<%=iPageCount %>&bilangan=<%=bil%>&ms=next&drujuk=<%=rujuk%>&dnama=<%=nama%>&dadd=<%=add%>&proses=Cari">
...
<td onMouseover="this.style.backgroundColor='CC6666'" onMouseout="this.style.backgroundColor='FFE9E8'"><a href="ik26b.asp?kod=<%=objRs2("rowid")%>"><%=objRs2("no_akaun")%></a>&nbsp;</td>
<%	kara = objRs2("perkara")
	if kara <> "" then	%>
...
<% noruj= objRs2("no_akaun")
 ee = "select to_char(sysdate,'dd/mm/yyyy') tkh from dual "
		set rsee = objconn.execute(ee)	
		
		tkh=rsee("tkh")

gg = " select stesyen as stesyen1, nvl(amaun_bayar,0) am1,to_char(tarikh,'yyyy') yy1,to_char(tarikh,'mm') mm1 ,to_char(tarikh,'dd') dd1, no_resit as no_resit1 "
    gg = gg & " from kutipan.kutipan where no_akaun = '"& noruj &"' and (status <> 'B' or status is null) and post is null "
	
	set objff = objconn.execute(gg)
	''response.Write(gg)
	if not objff.eof then 	
	
	stat = "kutip"
	stesyen1 = objff("stesyen1")
	amaun1 = objff("am1")
	yy1 = objff("yy1")
	mm1 = objff("mm1")
	dd1 = objff("dd1")
	tkhq1 = cstr(dd1)+"/"+cstr(mm1)+"/"+cstr(yy1)
	resitq1 = objff("no_resit1")
	end if
	
	if stat = "" then
		ff = " select 'MPSPPAY' stesyen, nvl(amaun,0) am,to_char(tarikh,'yyyy') yy,to_char(tarikh,'mm') mm,"
			ff = ff & " to_char(tarikh,'dd')dd,trx_id, no_resit "
			ff = ff & " from hasil.ebayar_trxid where no_akaun = '"& noruj &"' and flag = 'SUCCESSFUL' and status_kutipan is null "
 ...
 if stat = "kutip" then
 	...
 	ayat = amaun1
 else if stat = "mpsppay" then
 	ayat = "Bayaran Telah Dibuat Pada "&tkhq1&"<br>di Stesyen "&stesyen&". Jumlah: RM "&amq&". No Resit: "&resitq&". Data Belum Dikemaskini "
 else
 	ayat = "kosong"
 end if
 end if
