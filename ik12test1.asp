<%response.cookies("ikmenu") = "ik12test1.asp"%>
<!-- '#INCLUDE FILE="ik.asp" -->
...
<form name=komp method="POST" action="ik12test1.asp">
<%	
...
'****************************************** SUB FORM ***************************************************	
 ...
 <table width="50%" cellpadding="1" cellspacing="5" class="hd1">
    ...
    <td colspan="7"  class="hd1" align="right">
      ...
      <a href="ik12test1.asp?page=1&bilangan=0&ms=pre&dtkh=<%=ftkh%>&tkhdr=<%=tkhd%>&tkhhg=<%=tkhh%>&b2=Cari">
		<img name="firstrec" border="0" src="images/firstrec.jpg" width="20" height="20" alt="Halaman Mula"></a>  
      <% End If %>
      <% If iPageCurrent <> 1 Then%>
      <a href="ik12test1.asp?page=<%= iPageCurrent - 1 %>&bilangan=<%=bil-count-iPageSize%>&ms=pre&dtkh=<%=ftkh%>&tkhdr=<%=tkhd%>&tkhhg=<%=tkhh%>&b2=Cari">
      ...
      <a href="ik12test1.asp?page=<%= iPageCurrent + 1 %>&bilangan=<%=bil%>&ms=next&dtkh=<%=ftkh%>&tkhdr=<%=tkhd%>&tkhhg=<%=tkhh%>&b2=Cari">
      	<img name="next" border="0" src="images/next.jpg" width="20" height="20" alt="Rekod Seterusnya"></a> 
      <% End If 
	  If cint(iPageCurrent) < cint(iPageCount) Then
	  bil = (iPageCount - 1) * iPageSize %>
      <a href="ik12test1.asp?page=<%=iPageCount %>&bilangan=<%=bil%>&ms=next&dtkh=<%=ftkh%>&tkhdr=<%=tkhd%>&tkhhg=<%=tkhh%>&b2=Cari">
...
<input type="submit" value="Cetak" name="B1" onClick="this.form.action='ik12ntest.asp?tkhd=<%=tkhd%>&tkhh=<%=tkhh%>&ftkh=<%=ftkh%>';" class="button">
</td>
</tr></table>
<%		end if
		end if		
