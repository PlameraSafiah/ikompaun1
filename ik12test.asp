<%response.cookies("ikmenu") = "ik12test.asp"%>
...
<form name=komp method="POST" action="ik12test.asp">
<%	

response.cookies("gnop") = "17658"
pekz = "17658"
...
'*********************************************************************************************************
  ...
  if b2 = "Cari" or b4 = "Cari" or proses = "Hantar" then  
	...
	if b3 = "Cetak" then
		...
		'Kompaun Pelesenan
		c = c & " and (no_akaun like '76410'||'%' "
		'Kompaun Veterinar & kesihatan
		c = c & "or no_akaun like '76420'||'%' or no_akaun like '76413'||'%' "
		'Kompaun Bangunan
		c = c & " or no_akaun like '76101'||'%'  "
		'Kompaun Perbandaran
		c = c & " or no_akaun like '76412'||'%' or no_akaun like '76415'||'%' ) "
       ...
       if not isNull(rs4("amaun_bayar")) then	    
		...
		response.write "<script language=""javascript"">"
       	...
       	response.write "</script>" 
			...
			response.redirect "ik12n2test.asp"
		end if	
		end if
	end if	
'************************************* SUB MULA *********************************************************
...
'****************************************** SUB FORM ***************************************************	
	...
	sub form
		...
		'Kompaun Veterinar & kesihatan
		d = d & "or no_akaun like '76420'||'%' or no_akaun like '76413'||'%' "
		'Kompaun Bangunan
 ...
 <table width="50%" cellpadding="1" cellspacing="5" class="hd1">
    ...
    <td colspan="7"  class="hd1" align="right">
	  <% If iPageCurrent <> 1 Then %>
      <a href="ik12test.asp?page=1&bilangan=0&ms=pre&dtkh=<%=ftkh%>&tkhdr=<%=tkhd%>&tkhhg=<%=tkhh%>&b2=Cari">
		<img name="firstrec" border="0" src="images/firstrec.jpg" width="20" height="20" alt="Halaman Mula"></a>  
      <% End If %>
      <% If iPageCurrent <> 1 Then%>
      <a href="ik12test.asp?page=<%= iPageCurrent - 1 %>&bilangan=<%=bil-count-iPageSize%>&ms=pre&dtkh=<%=ftkh%>&tkhdr=<%=tkhd%>&tkhhg=<%=tkhh%>&b2=Cari">
      ...
      <a href="ik12test.asp?page=<%= iPageCurrent + 1 %>&bilangan=<%=bil%>&ms=next&dtkh=<%=ftkh%>&tkhdr=<%=tkhd%>&tkhhg=<%=tkhh%>&b2=Cari">
      	<img name="next" border="0" src="images/next.jpg" width="20" height="20" alt="Rekod Seterusnya"></a> 
      <% End If 
	  If cint(iPageCurrent) < cint(iPageCount) Then
	  bil = (iPageCount - 1) * iPageSize %>
      <a href="ik12test.asp?page=<%=iPageCount %>&bilangan=<%=bil%>&ms=next&dtkh=<%=ftkh%>&tkhdr=<%=tkhd%>&tkhhg=<%=tkhh%>&b2=Cari">
  ...
  <td width="6%" class="hd1">Tarikh Cetakan Terakhir</td>
   <td width="6%" class="hd1">Tarikh Notis </td>
  <td width="8%" class="hd1" >Notis</td>
    ...
    <% 	bil = 0
    	...
    	if tnotis3 <> "" then
    		bilnotis = 3
			tarikh_notis = tnotis3
    	elseif tnotis2 <> "" then
    		bilnotis = 2
			tarikh_notis = tnotis2
    	elseif tnotis1 <> "" then
    		bilnotis = 1
			tarikh_notis = tnotis1
    	else
    		bilnotis = "" 	
			tarikh_notis = ""
    	end if		    	
...
<input type="submit" value="Cetak" name="B1" onClick="this.form.action='ik12ntst.asp?tkhd=<%=tkhd%>&tkhh=<%=tkhh%>&ftkh=<%=ftkh%>';" class="button">
</td>
</tr></table>
<%		end if
		end if		
