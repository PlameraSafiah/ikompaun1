...
<%	
	...
	if tkhd = "" and proses <> "Cari" or ftkh <> "" then	
	...
	bilcount = Request.form("bilrec")	
		...
		if vpost = "N" then
			if vtnotis2 <> "" then
				...
				r = " update hasil.bil set post = '"&vpost&"', tarikh_post = to_date('"&sisdate&"','ddmmyyyy'), " ''lama pnya : tarikh_post = to_date('"&vtkh&"','ddmmyyyy'),
				r = r & " tarikh_notis3 = to_date('"&sisdate&"','ddmmyyyy'),cetak='Y' "
				r = r & " where rowid = '"&vrowid&"' "

				Set objRs3 = objConn.Execute(r)
			elseif vtnotis1 <> "" and vtnotis2 = "" then

				r2 = " update hasil.bil set post = '"&vpost&"', tarikh_post = to_date('"&sisdate&"','ddmmyyyy'), "
				r2 = r2 & " tarikh_notis2 = to_date('"&sisdate&"','ddmmyyyy'),cetak = 'Y' "
				r2 = r2 & " where rowid = '"&vrowid&"' "
				Set objRs32 = objConn.Execute(r2)
			elseif vtnotis1 = "" then

				r3 = " update hasil.bil set post = '"&vpost&"', tarikh_post = to_date('"&sisdate&"','ddmmyyyy'), "
				r3 = r3 & " tarikh_notis1 = to_date('"&sisdate&"','ddmmyyyy'),cetak='Y' "
...
<td class="hd">
  ...
  <tr> 
<td class="hd">No Kompaun :</td><td>
          <input name="ftkh" type="text" id="ftkh" value="<%=ftkh%>" size="13" maxlength="12"> <%'tarikh notis tukaq jadi no kompaun -mimi %>
...
'****************************************** SUB FORM ***************************************************	
	...
	if ftkh <> "" then '----ftkh to no kompaun : 27072018
		...
		d = d & " or no_akaun like '76415'||'%' "
		d = d & " or no_akaun like '76101'||'%' )" 'or no_akaun like '76441'||'%') "		d = d & " and tkh_bayar is null "
		d = d & " and perkara <> 'P01'  "
		d = d & " and post <> 'M' "
		...
		d = d & " to_char(tkh_masuk,'ddmmyyyy') as tkh_masuk, post, tarikh_notis1, tarikh_notis2, tarikh_notis3 "
     	d = d & " from hasil.bil where"
		d = d & " tkh_masuk between to_date('"&tkhd&"','ddmmyyyy') and to_date('"&tkhh&"','ddmmyyyy')"
		d = d & " and (no_akaun like '76410'||'%' or no_akaun like '76420'||'%' or no_akaun like '76413'||'%' "
		d = d & " or no_akaun like '76415'||'%' "
		d = d & " or no_akaun like '76101'||'%' )" 'or no_akaun like '76441'||'%') "		d = d & " and tkh_bayar is null "
