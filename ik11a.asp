...
'   objConn.Open "dsn=12c;uid=majlis;pwd=majlis;"   
	...
	noakaun = request.QueryString("noakaun1")	
...
'***************************************** LIST BUTIR KESALAHAN *****************************************		
 ...
 '******************************************** PROSES SAVE	 *************************************************
	...
	response.Write "Simpan sini :: " &(proses1) & "<br>" 
	fperkara4 = ucase(Request.Form("fperkara4"))
	fperkara5 = ucase(Request.Form("fperkara5"))
	fnama = ucase(Request.Form("fnama"))
	falamat1 = ucase(Request.Form("falamat1"))
	...
	if not objRsnn.eof then
			...
			d = d & " jabatan = '"& jab &"',kategori = '"& kat &"',akaun = '"& dakaun &"',amaun='"&famaun&"', parlimen='"&fparlimen1&"', "
			d = d & " daerah = '"&fdaerah1&"',dun = '"&fdun1&"', kod_ternakan = '"&fternakan1&"' "
		   ...
		   response.redirect "ik11_edit.asp?noakaun="&noakaun&""
			...
			d = d & " '"& fno_pekerja &"','"&jab&"', '"&kat&"', '"&dakaun&"','"&famaun&"','"&fparlimen1&"','"&fdaerah1&"','"&fdun1&"','"&fternakan1&"')" 	
	...
	Set gq = objConn.Execute(s)
  		...
  		if not gq.eof then
			...
			rowid = gq("rowid")
			perkara4 = gq("perkara4")
			perkara5 = gq("perkara5")
			nama = gq("nama")
			alamat1 = gq("alamat1")
			alamat2 = gq("alamat2")
			alamat3 = gq("alamat3")
			...
			tkh_masuk = gq("tkh_masuk")
			waktu1 = gq("masa")
			perkara1 = gq("perkara1")
			perkara2 = gq("perkara2")
			ftred = gq("tred")
			fjenis = gq("jenis")
...
'	end if	
  ...
  <% 
   ...
   <tr> 
          ...
          <td> <input type="text" name="fnama" size="60" value="<%if fnama = "" then%> <%=nama%> <% else %> <%=fnama%> <%end if%>" maxlength="40" ></td>
        </tr>
...
<td rowspan="7" class="hd"> Alamat <font size="1" color="#FF00FF">*</font></td> 
    <tr> 
          <td> 
		  <input type="text" name="falamat1" size="60" value="<%if falamat1 = "" then%> <%=alamat1%> <% else %> <%=falamat1%> <%end if%> " maxlength="40" > 
		 	  </td>
    </tr>
        <tr> 
          <td> <input type="text" name="falamat2" size="60" value="<%if falamat2 = "" then%> <%=alamat2%> <% else %> <%=falamat2%> <%end if%> " maxlength="40" > 
            </td>
        </tr>
        <tr> 
          <td> <input type="text" name="falamat3" size="60" value="<%if falamat3 = "" then%> <%=alamat3%> <% else %> <%=falamat3%> <%end if%> " maxlength="40" > 
            </td>
        </tr>
    ...
    <td >DAERAH : 
	 ...
	 do while not rszz1.eof 
		   
		   koddaerah = rszz1("daerah")
...
<tr> 
          ...
          <td> <input type="text" name="fperkara4" size="13" value="<%if fperkara4 = "" then%> <%=perkara4%> <% else %> <%=fperkara4%> <%end if%>" maxlength="40" >
	</td>
        </tr>
        <tr>
          <td class="hd">No Sykt / Daftar</td>
          <td> <input type="text" name="fperkara5" size="13" value="<%if fperkara5 = "" then%> <%=perkara5%> <% else %> <%=fperkara5%> <%end if%>" maxlength="40" ></td>
        </tr>
        ...
        <tr> 
          <td  class="hd">No 
            K/P</td>
          <td> <input type="text" name="fkp" size="20" value="<%if fkp = "" then%> <%=kp%> <% else %> <%=fkp%> <%end if%>" maxlength="14" onKeyPress="return isNumberKey(event)" ></td>
        </tr>
        <%	if tkh_lahir = "" then
   			if Len(kp) >= 12 then
...
%>
        <tr> 
          ...
          <td> <input type="text" name="ftkh_masuk" size="10" value="<%if ftkh_masuk = "" then%> <%=tkh_masuk%> <% else %> <%=ftkh_masuk%> <%end if%>" maxlength="8" onKeyUp="DateFormat(this)">
            &nbsp;<font face="Arial" size="1" color="#244980"><b>(ddmmyyyy)</b></font></td>
            </tr>
            <tr>
          <td class="hd">Waktu <font size="1" color="#FF00FF">*</font></td>
          <td> <input type="text" name="fmasa" value="<%if fwaktu1 = "" then%> <%=waktu1%> <% else %> <%=fwaktu1%> <%end if%>" size="5" maxlength="5" >
           
            &nbsp; <font size="1" face="Arial"><b> AM
            <input type="radio" value="AM" name="fampm" <%if fwaktu1 < 13 then%>checked<%end if%>>
            &nbsp; PM
            <input type="radio" value="PM" name="fampm" <%if fwaktu1 > 12 then%>checked<%end if%>>
            </b></font></td>
        </tr>
        <tr> 
          <td class="hd">Tempat <font size="1" color="#FF00FF">*</font></td>
          <td> <input type="text" name="fperkara1" size="80" value="<%if fperkara1 = "" then%> <%=perkara1%> <% else %> <%=fperkara1%> <%end if%>" maxlength="100" ></td>
        </tr>
        <tr> 
          <td  class="hd">&nbsp;</td>
          <td> <input type="text" name="fperkara2" size="80" value="<%if fperkara2 = "" then%> <%=perkara2%> <% else %> <%=fperkara2%> <%end if%>" maxlength="100" > 
  ...
  %>
          <td> <input type="text" name="fno_pekerja" size="7" value="<%if fno_pekerja = "" then%><%=no_pekerja%><% else %><%=fno_pekerja%><%end if%>" maxlength="5" onFocus="nextfield='B1';">
